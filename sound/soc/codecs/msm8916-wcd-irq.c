			...
			const char *name, void *priv)
{
	...
	map.linuxirq[irq] =
		platform_get_irq_byname(map.pdev[BIT_BYTE(irq)], irq_names[irq]);
	...
	rc = devm_request_threaded_irq(&map.pdev[BIT_BYTE(irq)]->dev,
				map.linuxirq[irq], NULL,
				wcd9xxx_spmi_irq_handler,
				irq_flags,
				name, priv);
		if (rc < 0) {
			dev_err(&map.pdev[BIT_BYTE(irq)]->dev,
				"Can't request %d IRQ\n", irq);
			return rc;
		}

	dev_dbg(&map.pdev[BIT_BYTE(irq)]->dev,
...
{
	devm_free_irq(&map.pdev[BIT_BYTE(irq)]->dev, map.linuxirq[irq],
						priv);
	map.mask[BIT_BYTE(irq)] |= BYTE_BIT_MASK(irq);
	return 0;
}
