...
static void smb1360_enable_wd(struct smb1360_chip *chip, bool enable)
{
	...
	rc = smb1360_masked_write(chip, WD_CFG_REG, WD_CFG_CTRL_BIT,
...
#ifdef CONFIG_ARCH_SONY_KANUTI
static int config_safety_time(int safety_time) {
	...
	if (chip->safety_time != -EINVAL) {
		if (chip->safety_time == 0) {
			...
			rc = smb1360_masked_write(chip, CFG_SFY_TIMER_CTRL_REG,
					SAFETY_TIME_DISABLE_BIT | SAFETY_TIME_MINUTES_MASK, reg);
			if (rc < 0) {
				dev_err(chip->dev,
					"Couldn't set safety timer rc = %d\n", rc);
				return rc;
...
static void brain_work(struct work_struct *work)
{
	...
	if (count_down) {
		if (ibat_ma > 0 && soc_delta > 0 &&
				steal_soc_counter >= STEAL_SOC) {
			soc_delta--;
			steal_soc_counter = 0;
			pr_info("soc_delta=%d\n", soc_delta);
			if (soc_delta == 0) {
