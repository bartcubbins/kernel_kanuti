	...
	while (read_count++ < CHECK_CHIP_ID_TIME_MAX) {
		...
		for (i = 0; i < bma2x2_sensor_type_count; i++) {
			if (sensor_type_map[i].chip_id == chip_id) {
				...
				dev_dbg(&client->dev,"Bosch Sensortec Device detected,"
					" HW IC name: %s\n",
		...
		if (i < bma2x2_sensor_type_count)
			...
			if (read_count == CHECK_CHIP_ID_TIME_MAX) {
				dev_err(&client->dev,"Failed! Bosch Sensortec Device"
	...
	if (mode < 6) {
		...
		case BMA2X2_MODE_SUSPEND:
		     if (bma2x2->bma_mode_enabled != 0) {
                dev_err(&client->dev,"bma still working");
                return 0;
                }
				data1  = BMA2X2_SET_BITSLICE(data1,
						BMA2X2_MODE_CTRL, 4);
		...
		const char *buf, size_t count)
{
...
#ifdef CONFIG_SENSORS_BMI058
	dev_dbg(&client->dev,"diff y is %d,value1 is %d, value2 is %d\n", diff,
				value1, value2);
	test_result_branch = 2;
#else
	dev_dbg(&client->dev,"diff x is %d,value1 is %d, value2 is %d\n", diff,
				value1, value2);
	...
	dev_dbg(&client->dev,"diff z is %d,value1 is %d, value2 is %d\n", diff,
			value1, value2);
	...
	bma2x2_soft_reset(bma2x2->bma2x2_client);
	mdelay(5);
	dev_dbg(&client->dev,"self test finished\n");
		...
		const char *buf, size_t count)
{
	...
	do {
		...
		/*dev_dbg(&client->dev,"wait 2ms cal ready flag is %d\n", tmp); */
		timeout++;
		if (timeout == 50) {
			dev_dbg(&client->dev,"get fast calibration ready error\n");
			return -EINVAL;
		};
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (data > MAX_FIFO_F_LEVEL) {
		dev_err(&client->dev,"bma2x2 fifo_count abnormal, %d", data);

		if (bma2x2_get_mode(bma2x2->bma2x2_client, &mode) < 0)
			return -EINVAL;

		dev_err(&client->dev,"bma2x2 fifo_count abnormal, op_mode: %d", mode);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bma2x2->bma_mode_enabled == 0) {
		dev_err(&client->dev,"bma2x2_fifo_data, bma_mode_enabled 0");
		/*return -EINVAL;*/
	}
...
static void bma2x2_tap_timeout_handle(unsigned long data)
{
	...
	dev_dbg(&client->dev,"tap interrupt handle, timeout\n");
...
static void bma2x2_high_g_interrupt_handle(struct bma2x2_data *bma2x2)
{
	...
	for (i = 0; i < 3; i++) {
		...
		dev_dbg(&client->dev,"High G interrupt happened,exis is %d,"
...
static void bma2x2_slope_interrupt_handle(struct bma2x2_data *bma2x2)
{
	...
	for (i = 0; i < 3; i++) {
		...
		dev_dbg(&client->dev,"Slop interrupt happened,exis is %d,"
					"first is %d,sign is %d\n", i,
						first_value, sign_value);
	}
}
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	if ((status&0x80) == 0x80) {
		/* dev_dbg(&client->dev,"New data interrupt happened\n");*/
	...
	dev_dbg(&client->dev,"bma2x2_irq_work_func, status = 0x%x\n", status);

#ifdef CONFIG_SIG_MOTION
	if (status & 0x04)	{
		if (atomic_read(&bma2x2->en_sig_motion) == 1) {
			dev_dbg(&client->dev,"Significant motion interrupt happened\n");
			/* close sig sensor,
	...
	if (status & 0x20) {
		if (atomic_read(&bma2x2->en_double_tap) == 1) {
			dev_dbg(&client->dev,"single tap interrupt happened\n");
	...
	case 0x01:
		dev_dbg(&client->dev,"Low G interrupt happened\n");
	...
	case 0x08:
		dev_dbg(&client->dev,"slow/ no motion interrupt happened\n");
		input_report_rel(bma2x2->dev_interrupt,
	...
	case 0x10:
		dev_dbg(&client->dev,"double tap interrupt happened\n");
		input_report_rel(bma2x2->dev_interrupt,
			DOUBLE_TAP_INTERRUPT,
			DOUBLE_TAP_INTERRUPT_HAPPENED);
		break;
	...
	case 0x40:
		bma2x2_get_orient_status(bma2x2->bma2x2_client,
				    &first_value);
		dev_dbg(&client->dev,"orient interrupt happened,%s\n",
				orient[first_value]);
		if (first_value == 0)
	...
	case 0x80:
		bma2x2_get_orient_flat_status(bma2x2->bma2x2_client,
				    &sign_value);
		dev_dbg(&client->dev,"flat interrupt happened,flat status is %d\n",
				    sign_value);
		if (sign_value == 1) {
			input_report_abs(bma2x2->dev_interrupt,
				FLAT_INTERRUPT,
...
//CONN-EC-SensorRemoveDev-01-	struct input_dev *dev_interrupt;
	...
	dev_dbg(&client->dev,"bma2x2_probe start\n");
	...
	if (bma2x2_soft_reset(client) < 0) {
		dev_err(&client->dev,"i2c bus write error, pls check HW connection\n");
		err = -EINVAL;
		goto kfree_exit;
	}
	mdelay(5);
...
//CONN-EC-SensorPorting-01+]
	...
	if (pdata) {
		if (pdata->irq_gpio_cfg && (pdata->irq_gpio_cfg() < 0)) {
			dev_err(&client->dev,"IRQ GPIO conf. error %d\n",
					client->irq);
		}
	}
	...
	if (NULL != client->dev.platform_data) {
		...
		if (NULL != data->bst_pd) {
			memcpy(data->bst_pd, client->dev.platform_data,
					sizeof(*data->bst_pd));
			dev_dbg(&client->dev,"%s sensor driver set place: p%d",
...
//CONN-EC-SensorPorting-01-]
	...
	if (bma2x2_set_mode(client, BMA2X2_MODE_SUSPEND, BMA_ENABLED_ALL) < 0)
		return -EINVAL;
	dev_dbg(&client->dev,"BMA2x2 driver probe successfully");
